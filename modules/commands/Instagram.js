  ...
  try {
    ...
    case "i":{
      ...
      if (!username) return api.sendMessage("con mẹ m info đâu", threadID);
       const res = await axios.get(`https://caochungdat.me/docs/instagram/info?username=${username}`);
...
var url = res.data.images[0].image_versions2.candidates[0].url;
    ...
    data = (await axios.get(`https://caochungdat.me/docs/instagram/videodl?url=${link_video}`)).data,
